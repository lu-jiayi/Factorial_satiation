var utils = {
	...
	make_likert_scale: function(label, response_callback, orientation) {
		var orientation = (orientation == null) ? "horizontal" : orientation;
		...
		$(label).slider({
			...
			max: 7,
			step: 1,
			value: 4,  // Set the default value to the midpoint
			slide: response_callback,
			change: response_callback,
			orientation: orientation
		});
		...
		// Show the handle and change colors on mouse down
		$(label).mousedown(function () {
			$(label + ' .ui-slider-handle').show();
			$(label).css({ "background": "#99D6EB" });
			$(label + ' .ui-slider-handle').css({
		...
		var likertLabels = ['1', '2', '3', '4', '5', '6', '7'];
		var likertContainer = $('<div>').css({
			display: 'flex',
			justifyContent: 'space-between',
			marginTop: '20px',
			width: '116%'  // Ensure the container spans the full width
		});
	
		// Update CSS for labels based on orientation
		var labelCss = {
			textAlign: orientation === "horizontal" ? 'left' : 'center',
			flex: '1'  // Make each label take up equal space
		};
	
		likertLabels.forEach(function (likertLabel) {
			var labelDiv = $('<div>').text(likertLabel).css(labelCss);
			likertContainer.append(labelDiv);
		});
